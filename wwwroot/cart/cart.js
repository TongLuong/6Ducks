$(document).ready(function () {
    ...
    checkboxes.forEach(function (checkbox) {
      checkbox.addEventListener("change", function () {
        ...
        if ($(".cash .left").has(this).length) {
          ...
          if (this.checked) $(".cash .right form").css("display", "");
          else $(".cash .right form").css("display", "none");
        }
      });
    });
  });
  ...
  var shippingPrice = {};
  $.get("Product/LoadShippingMethods", {}, function (response) {
    for (var i = 0; i < response.data.length; i++) {
      var temp = response.data[i].value;
  ...
  $("#method").on("change", function (e) {
    ...
    $(".total").val(
      Number($(".total-price span").text()) + Number($("#cost").val())
    );
  });
  ...
  $("#make-buy").click(function () {
    ...
    for (var i = 0; i < itemsChose.length; i++) {
      ...
      $(".buy-cart-item-" + itemsChose[i] + " img").attr(
        "src",
      ...
      for (var i = 0; i < itemsChose.length; i++) {
        if (!(itemsChoseSellerID[i] in sellerBillPair)) {
          $.ajax({
            ...
            async: false,
            success: function (response) {
              sellerBillPair[itemsChoseSellerID[i]] = response.billID;
            },
          });
        }
    ...
    $("#cancel-2").click(function () {
      $(".overlay-cart .voucher input").val("");
